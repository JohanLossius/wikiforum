...
const PostsPage: React.FC = () => {
  const [posts, setPosts] = useState<Post[]>([]);
  const [newPost, setNewPost] = useState({ title: '', body: '' }); // Using 'body'
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data: Post = await response.json();
      setPosts((prev) => [...prev, data]);
      setNewPost({ title: '', body: '' }); // Using 'body'
      setError(null);
    } catch (err) {
      setError('Failed to create post: ' + (err as Error).message);
    }
    ...
    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
      ...
      <form onSubmit={handleSubmit} style={{ marginBottom: '20px' }}>
        ...
        <div>
          <label>Content:</label>
          <textarea
            name="body" // Changed to 'body' to match state
            value={newPost.body} // Using 'body'
            onChange={handleInputChange}
            required
            style={{ margin: '10px', padding: '5px', width: '300px', height: '100px' }}
          />
